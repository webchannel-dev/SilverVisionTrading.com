...
function wpsb_show_form($rtn = 0) {	
	...
	if (is_array($wpsb_flds)) {
		foreach ($wpsb_flds as $wpsb_k => $wpsb_v) {
			if (is_numeric($wpsb_k) && $wpsb_v) {
				...
				$out .= ' <span class="textbox"><input type="text" name="wpsb_fld['. $wpsb_k .']" id="wpsb_fld_'. $wpsb_k .'"  maxlength="64" class="wpsb_form_txt" /></span></p>' . "\n";
			}
		}
	}
	$out .= '<p class="wpsb_form_label">' . get_option('wpsb_form_email');
	$out .= '<br /> <span class="textbox"><input type="text" name="wpsb_email" id="wpsb_email" class="wpsb_form_txt" /></span></p>' . "\n";
	...
	$out .= '<p class="wpsb_form_label"><input class="button" type="submit" value="' . get_option('<!--wpsb_form_send-->');
