...
Plugin URI: http://aarongloege.com/blog/web-development/wordpress/plugins/wordpress-plugin-category-templates/
...
function cat_temp_post_submit($post_ID) {
  ...
  if ($_POST['cat_temp_template']) {
		$templates = unserialize(get_option("cat_temp_post"));
		...
		update_option("cat_temp_post",serialize($templates));
  }
}
...
function cat_temp_get_data($archive=false, $id=false) {
	$t = (!$archive) ? unserialize(get_option('cat_temp_data')) : unserialize(get_option('cat_arch_data'));
	return (!$id) ? $t : $t[$id];
}
...
function cat_temp_update($data) {
	...
	unset($data['archive']);
	update_option('cat_temp_data', serialize($data));
	update_option('cat_arch_data', serialize($archive));
}
